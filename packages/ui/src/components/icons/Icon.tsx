...
export type IconSizeScale = 16 | 20 | 24 | 32 | 48;
/* Sizes used exclusively for the Checkbox component */
export type IconSizeScaleCheckbox = 14 | 18;

export type IconsProps = {
  name: Icons;
  color?: Colors;
  size?: IconSizeScale | IconSizeScaleCheckbox | '100%';
  testID?: string;
  accessible?: boolean;
...
change its color values. It accepts `Colors` values only.
*/
export const Icon = ({
  name,
  color = 'grey-700',
  ...
  const iconSize = allowFontScaling && typeof size === 'number' ? size : size;
...
class AnimatedIconClassComponent extends React.Component<AnimatedIconsProps> {
  // eslint-disable-next-line @typescript-eslint/no-useless-constructor
  constructor(props: AnimatedIconsProps) {
    super(props);
  }
  render() {
...
/* New icons */
const { sparkles, profileRegistered, documentAdd } = Icons;
...
export type NavIcons = keyof typeof NavIcons;

/* Biometric */
const { biomFingerprint, biomFaceID } = Icons;
...
export type BiometricIcons = keyof typeof BiometricIcons;
...
export type SystemIcons = keyof typeof SystemIcons;
