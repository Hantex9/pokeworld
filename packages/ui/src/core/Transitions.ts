import { Easing, withDelay, withTiming, WithTimingConfig } from 'react-native-reanimated';
...
export const enterTransitionAlertEdgeToEdge = (values: { targetHeight: number }) => {
  'worklet';
  const animations = {
    opacity: withTiming(1, {
...
*/
export const exitTransitionAlertEdgeToEdge = (values: { currentHeight: number }) => {
  'worklet';
  const animations = {
    opacity: withTiming(0, {
...
export const exitTransitionInputIcon = () => {
  ...
  const animations = {
    ...
    transform: [
      {
        scale: withTiming(iconTransitionScaleFactor, iconTransitionWithTimingConfig),
      },
    ],
  };
  const initialValues = {
